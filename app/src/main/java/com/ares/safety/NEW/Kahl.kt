...
data class Coordenada(val fila: Int = 0, val columna: Int = 0)
data class RangoNumerico(val inicio: Int = 0, val fin: Int = 0)
data class PersonaBasica(val nombre: String = "", val edad: Int = 0)
data class ProductoBasico(val clave: String = "", val precio: Double = 0.0)
data class EstadisticaSimple(
    val total: Int,
    val cantidad: Int,
    val promedio: Double
)

sealed class EstadoPasivo {
    data class Disponible(val descripcion: String) : EstadoPasivo()
    data class NoDisponible(val motivo: String) : EstadoPasivo()
}

object ModuloAuxiliarPasivo {
